...
# MongoDB를 사용하여 각 여정(일기, 생성된 이미지, 방명록, 큐레이터 메시지 등)을
...
class GalleryManager:
    ...
    def add_curator_message(self, item_id: str, curator_message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = self.gallery_items.update_one(
                {"item_id": item_id},
                {
                    "$set": {
                        ...
                        "gpt_curator_tokens": gpt_curator_tokens
                    }
                }
            )
            ...
            logger.error(f"큐레이터 메시지 추가 실패: {e}")
            return False
    ...
    def get_user_gallery(
        ...
        try:
            ...
            cursor = self.gallery_items.find(query).sort("created_date", -1).skip(offset).limit(limit)
    ...
    def _doc_to_gallery_item(self, doc: Dict[str, Any]) -> GalleryItem:
        ...
        return GalleryItem(
            ...
            curator_message=doc.get("curator_message", {}),
            ...
            gpt_curator_used=doc.get("gpt_curator_used", True),
            gpt_curator_tokens=doc.get("gpt_curator_tokens", 0),
            prompt_generation_time=doc.get("prompt_generation_time", 0.0),
            prompt_generation_method=doc.get("prompt_generation_method", "gpt"),
            curator_generation_method=doc.get("curator_generation_method", "gpt"),
        )
        ...
        try:
            ...
            pipeline = [
                ...
                {"$group": {
                    ...
                    "gpt_curators": {"$sum": {"$cond": ["$gpt_curator_used", 1, 0]}},
                    "total_prompt_tokens": {"$sum": "$gpt_prompt_tokens"},
                    "total_curator_tokens": {"$sum": "$gpt_curator_tokens"},
            ...
            prompt_methods = {doc["_id"]: doc["count"] for doc in self.gallery_items.aggregate(prompt_method_pipeline)}
            
            curator_method_pipeline = [
                {"$match": {"user_id": user_id, "curator_message": {"$ne": {}}}},
                {"$group": {"_id": "$curator_generation_method", "count": {"$sum": 1}}}
            ]
            curator_methods = {doc["_id"]: doc["count"] for doc in self.gallery_items.aggregate(curator_method_pipeline)}
            ...
            return {
                ...
                "gpt_usage": {
                    ...
                    "curator_generations": stats["gpt_curators"],
                    "full_gpt_journeys": min(stats["gpt_prompts"], stats["gpt_curators"]),
                },
                "token_usage": {
                    "total_tokens": stats["total_prompt_tokens"] + stats["total_curator_tokens"],
                    "prompt_tokens": stats["total_prompt_tokens"],
                    "curator_tokens": stats["total_curator_tokens"],
                    "avg_tokens_per_item": (
                        (stats["total_prompt_tokens"] + stats["total_curator_tokens"]) / stats["total_items"]
                    ),
                },
                "performance": {
                    "avg_generation_time": stats["avg_generation_time"] or 0.0,
                    "gpt_adoption_rate": stats["gpt_prompts"] / stats["total_items"],
                    "completion_rate": stats["gpt_curators"] / stats["gpt_prompts"] if stats["gpt_prompts"] > 0 else 0.0,
                },
                "method_distribution": {
                    "prompt_methods": prompt_methods,
                    "curator_methods": curator_methods,
                },
                "timeline": {
    ...
    def record_gallery_visit(
        ...
        try:
            ...
            logger.info(f"미술관 방문 기록: 사용자 {user_id}, 아이템 {item_id}, 유형 {visit_type}")
    ...
    def get_gallery_analytics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 감정 분포 데이터
            vad_docs = list(self.gallery_items.find({"user_id": user_id}, {"vad_scores": 1}))
            vad_data = [doc["vad_scores"] for doc in vad_docs if "vad_scores" in doc]
            
            # 방명록 제목들
            title_docs = list(self.gallery_items.find(
    ...
    def get_message_reaction_analytics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            reaction_counts = {doc["_id"]: doc["count"] for doc in self.message_reactions.aggregate(reaction_pipeline)}
            ...
            daily_pipeline = [
                {"$match": {"user_id": user_id}},
                {"$group": {
                    "_id": {"$dateToString": {"format": "%Y-%m-%d", "date": {"$dateFromString": {"dateString": "$reaction_date"}}}},
                    "count": {"$sum": 1}
                }},
                {"$sort": {"_id": -1}},
                {"$limit": 30}
            ]
            daily_reactions = {doc["_id"]: doc["count"] for doc in self.message_reactions.aggregate(daily_pipeline)}
            
            # 긍정적 반응률 계산
            total_reactions = sum(reaction_counts.values())
            positive_reactions = sum(reaction_counts.get(rt, 0) for rt in ["like", "save", "share"])
            ...
            return {
                ...
                "positive_reaction_rate": positive_reactions / total_reactions if total_reactions > 0 else 0,
                "daily_reaction_pattern": daily_reactions,
                "engagement_level": self._calculate_engagement_level(total_reactions, positive_reactions),
    ...
    def _calculate_gpt_message_performance(self, gpt_reaction_data: Dict) -> Dict[str, Any]:
        """GPT 메시지 성능 계산"""
        performance = {}
        ...
        if "gpt" in performance:
            ...
            performance["gpt_effectiveness"] = {
                ...
                "user_satisfaction": (
                    "high" if gpt_perf["positive_rate"] > 0.7
                    else "medium" if gpt_perf["positive_rate"] > 0.5 else "low"
                ),
                "recommendation": self._get_gpt_performance_recommendation(gpt_perf["positive_rate"]),
            }
    ...
    def _get_gpt_performance_recommendation(self, positive_rate: float) -> str:
        ...
        if positive_rate > 0.8:
            ...
            return "GPT 메시지 품질 개선이 필요합니다. 프롬프트 엔지니어링을 검토하세요."
        else:
            return "GPT 메시지 성능이 낮습니다. 시스템 설정과 안전성 검증을 점검하세요."

    def _calculate_engagement_level(self, total_reactions: int, positive_reactions: int) -> str:
        """참여도 수준 계산"""
    ...
    def _analyze_emotion_trends(self, vad_data: List[List[float]]) -> Dict[str, Any]:
        ...
        return {
            "valence": {
                ...
                "trend": "improving" if len(valences) > 1 and valences[-1] > valences[0] else "stable",
            },
            "arousal": {
            ...
            "dominance": {
                ...
                "recent_change": dominances[-1] - dominances[0] if len(dominances) > 1 else 0,
            },
        }
    ...
    def _analyze_title_sentiments(self, titles: List[str]) -> Dict[str, Any]:
        ...
        positive_words = {"light", "bright", "hope", "peace", "joy", "calm", "beautiful"}
    ...
    def _generate_growth_insights(self, vad_data: List[List[float]]) -> List[str]:
        ...
        if recent_valence > initial_valence + 0.1:
            insights.append("GPT 개인화를 통해 감정 상태가 전반적으로 개선되고 있습니다.")
        ...
        # 변동성 분석
        valence_var = sum((d[0] - recent_valence) ** 2 for d in vad_data) / len(vad_data)
        if valence_var < 0.1:
    ...
    def export_user_gallery(self, user_id: str, export_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for item in items:
                if item.reflection_image_path and Path(item.reflection_image_path).exists():
                    try:
                        ...
                        logger.warning(f"이미지 복사 실패: {item.reflection_image_path}, {e}")
            ...
            result = {
                ...
                "total_gpt_tokens": gpt_analytics.get("token_usage", {}).get("total_tokens", 0),
            }
            
            logger.info(f"사용자 {user_id}의 미술관 데이터 내보내기 완료 (MongoDB): {export_path}")
            return result
    ...
    def cleanup_old_items(self, days_old: int = 365) -> int:
        ...
        try:
            ...
            # 삭제할 아이템들의 이미지 경로 조회
            old_items = list(self.gallery_items.find(
                {"created_date": {"$lt": cutoff_date}},
                {"reflection_image_path": 1}
            ))
            
            image_paths = [item.get("reflection_image_path") for item in old_items if item.get("reflection_image_path")]
            
            # 데이터베이스에서 삭제
            delete_result = self.gallery_items.delete_many({"created_date": {"$lt": cutoff_date}})
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_items": total_items,
                "gpt_adoption_rate": fully_gpt_items / total_items if total_items > 0 else 1.0,
